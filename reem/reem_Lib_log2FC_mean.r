# ============================================================
# Reem × ChIP-seq sequence matching  — Lib_log2FC_mean
#
# BEFORE RUNNING:
...
# ── SAVE CSVs ─────────────────────────────────────────────────────────────────
write_csv(exact_all, file.path(out_dir, "reem_Lib_exact20bp_matches.csv"))
write_csv(core_all,  file.path(out_dir, "reem_Lib_core14bp_matches.csv"))
cat("\nCSVs saved.\n")
...
make_plot <- function(df, title_suffix) {
  ...
  ggplot(df, aes(x = peak_score, y = y_var, colour = tier, shape = tissue)) +
    ...
    labs(
      ...
      y        = "Lib_log2FC_mean",
      title    = "HNF1A Lib_log2FC_mean vs. ChIP-seq peak score",
...
if (!is.null(p_exact)) {
  print(p_exact)
  ggsave(file.path(out_dir, "reem_Lib_scatter_exact20bp.pdf"), p_exact, width = 7, height = 5)
}
if (!is.null(p_core)) {
  print(p_core)
  ggsave(file.path(out_dir, "reem_Lib_scatter_core14bp.pdf"), p_core, width = 7, height = 5)
}
